...
Blog::Base::Sql - Klasse zur Generierung von SQL
...
Das Programm
    ...
    my $sql = Blog::Base::Sql->new('Oracle');
...
sub dbmsNames {
    my $this = shift;
    return wantarray? @DbmsNames: bless \@DbmsNames,'Blog::Base::Array';
}
...
auf den das Objekt instanziiert ist.
...
So verhält es sich im Detail:
    ...
    $script = Blog::Base::Sql->stmtListToScript(
        '-- TEXT1',
        'STMT1',
        "STMT2\n...',
        "STMT3 (\n....\n)",
...
eindeutigen Wert (nur SQLite und MySQL).
...
sub createFunction {
    ...
    my $name = shift;
    my $body = Blog::Base::String->removeIndent(shift);
...
Generiere INSERT-Statement für Tabelle $table mit den
Kolumnen und Werten @keyVal bzw. @keys,@vals und liefere dieses zurück.

Schlüssel/Wert-Paare ohne Wert (Leerstring, undef) werden
ausgelassen. Damit ist sichergestellt, dass der Defaultwert der
Kolumne verwendet wird, sofern ein solcher auf der Datenbank
deklariert ist.
...
können per SQL berechnetet Kolumnenwerte eingesetzt werden.
...
sub insert {
    ...
    if (ref $_[0]) {
        if (Scalar::Util::blessed($_[0]) &&
                $_[0]->isa('Blog::Base::Dbms::Row::Object')) { # $row
            my $row = shift;
...
sub opIN {
    ...
    if (@_ == 1 && $_[0] =~ /SELECT/i) {
        # Subselect
        $str = Blog::Base::String->removeIndent($_[0],-addNL=>1);
        $str =~ s/^/        /mg;
        return "IN (\n$str    )";
    }
...
sub whereExpr {
    ...
    elsif (Blog::Base::Scalar->isArrayRef($expr)) {
        return $opExpr;
    }
    else {
        return "= $opExpr";
...
sub fromClause {
    ...
    for my $expr (@from) {
        ...
        $expr = Blog::Base::String->removeIndent($expr);
        $expr =~ s/\n/\n    /g;
    }
...
sub whereClause {
    ...
    while (@_) {
        if ($_[0] =~ /[\s=><]/) {
            ...
            my $clause = Blog::Base::String->removeIndent(shift);
            if ($clause =~ /\n/) {
...
sub diff {
    ...
    my @opt = Blog::Base::Misc->argExtractOpts(\@_);
    my @where = (@_,'('.join(" OR\n    ",@or).')');
