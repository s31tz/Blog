...
Blog::Base::FileHandle - Dateihandle
...
Datei schreiben:
    ...
    my $fh = Blog::Base::FileHandle->new('>',$path);
    $fh->print("Test\n");
    $fh->close;
...
Zähler-Datei mit Locking:
    ...
    my $fh = Blog::Base::FileHandle->open('+>>',$file,-lock=>'EX');
    $fh->seek(0);
    my $count = <$fh> || "0\n";
    chomp $count;
    $fh->truncate;
...
Siehe auch Blog::Base::LockedCounter.

=head1 METHODS

=head2 Konstruktor

=head3 new() - Instantiiere Dateihandle-Objekt
...
Filehandle-Objekt für STDOUT:
    ...
    $fh = Blog::Base::FileHandle->new(\*STDOUT);
...
sub new {
    ...
    if (ref $_[0] eq 'GLOB') {      # GLOB-Referenz
        ...
        if ($mode eq '<' && (!$path || $path eq '-')) {
            ...
            if (ref($path) &&
                    Blog::Base::Scalar->refType($path) eq 'SCALAR' &&
                    ref($path) ne 'SCALAR') {
...
Hebe Lock auf Datei $fh auf. Die Methode liefert keinen Wert zurück.
...
=head3 getc() - Liefere nächstes Zeichen
...
Lies gesamten Inhalt einer Datei ab Position $pos:
    ...
    my $fh = Blog::Base::FileHandle->open('<',$logFile);
    $fh->seek($pos);
    my $data = $fh->slurp;
...
=head3 binmode() - Schalte Filehandle in Binärmodus
