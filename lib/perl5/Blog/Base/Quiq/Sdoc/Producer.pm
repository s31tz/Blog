...
Blog::Base::Quiq::Sdoc::Producer - Sdoc-Generator
...
package Blog::Base::Quiq::Sdoc::Producer;
...
Generiere Sdoc mit Einrückung 2:
  ...
  $gen = Blog::Base::Quiq::Sdoc::Producer->new(
      indentation => 2,
  );
...
sub link {
    ...
    my $linkA = $self->linkA;
    push @$linkA,Blog::Base::Quiq::Hash->new({@_},
        name => $name,
        url => undef,
    );
...
sub table {
    ...
    if (ref($_[0]) || $_[0] =~ /^\d+$/) {
        ...
        $body = Blog::Base::Quiq::Table->new($arg1,$rowsA)->asAsciiTable;
    }
    else {
        $body = Blog::Base::Quiq::Unindent->trimNl(shift);
    }
