...
Blog::Base::Quiq::Terminal - Ein- und Ausgabe aufs Terminal
...
package Blog::Base::Quiq::Terminal;
...
Code die (langsame) Antwortzeit des Benutzers herauszunehmen.
  ...
  ...
  Blog::Base::Quiq::Terminal->askUser($prompt,
      -timer=>\$t0,
      ...
  );
  ...
...
Eingabe vom Terminal statt von STDIN per Filehandle:
  ...
  my $tty = Blog::Base::Quiq::FileHandle->new('<','/dev/tty');
  my $val = Blog::Base::Quiq::Terminal->askUser($prompt,-inHandle=>$tty);
  $tty->close;

Dasselbe per Option:

  my $val = Blog::Base::Quiq::Terminal->askUser($prompt,-ttyIn=>1);
...
sub askUser {
    ...
    while (1) {
        if ($ttyOut) {
            $out = Blog::Base::Quiq::FileHandle->new('>','/dev/tty');
        }
        print $out $prompt;

        if ($ttyIn) {
            $in = Blog::Base::Quiq::FileHandle->new('<','/dev/tty');
        }
...
Rote Schrift:
  ...
  $esc = Blog::Base::Quiq::Terminal->ansiEsc('red');

Fette weiße Schrift auf rotem Grund:

  $esc = Blog::Base::Quiq::Terminal->ansiEsc('bold white on_red');

Terminal in den Anfangszustand zurückversetzen:

  $esc = Blog::Base::Quiq::Terminal->ansiEsc('reset');
