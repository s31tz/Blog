...
Liefere aktuelles Verzeichnis:
  ...
  $dir = Blog::Base::Quiq::Process->cwd;

Wechsele Verzeichnis:

  Blog::Base::Quiq::Process->cwd('/tmp');
...
Liefere aktuelle EUID:
  ...
  $uid = Blog::Base::Quiq::Process->euid;

Setze EUID:

  Blog::Base::Quiq::Process->euid(1000);
...
sub user {
    my $this = shift;
    return Blog::Base::Quiq::System->user($>);
}
