...
sub new {
    ...
    my $ohash = Blog::Base::Quiq::Hash::Ordered->new;
    while (@_) {
        my $key = shift;
        my $val = shift;
...
Querystring mit Kaufmanns-Und als Trennzeichen:
  ...
  $url = Blog::Base::Quiq::Url->queryEncode(a=>1,b=>2,c=>3);
  =>
  a=1&b=2&c=3

Querystring mit Semikolon als Trennzeichen:

  $url = Blog::Base::Quiq::Url->queryEncode(-separator=>';',d=>4,e=>5);
  =>
  d=4;e=5
...
sub queryDecode {
    ...
    my $arr = Blog::Base::Quiq::Array->new;
    for (split /[&;]/,$str) {
        next if !$_;
        my ($key,$val) = split /=/;
