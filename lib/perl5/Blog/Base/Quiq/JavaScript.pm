...
=head4 Example
  ...
  $js = Blog::Base::Quiq::JavaScript->code(q°
...
sub code {
    ...
    return Blog::Base::Quiq::Template->combine(
        placeholders => \@_,
        template => $text,
    );
}
...
=head4 Synopsis
  ...
  $html = Blog::Base::Quiq::JavaScript->script($h,@specs);
...
Code zum Laden einer JavaScript-Datei über URL:
  ...
  $html = Blog::Base::Quiq::JavaScript->script($h,'https://host.dom/scr.js');
  =>
  <script src="https://host.dom/scr.js" type="text/javascript"></script>

Code aus Datei einfügen:

  $html = Blog::Base::Quiq::JavaScript->script($h,'inline:js/script.css');
  =>
...
Code direkt einfügen:
  ...
  $html = Blog::Base::Quiq::JavaScript->script($h,q|
      ...
  |);
  =>
  <script type="text/javascript">
...
Mehrere Code-Spezifikationen:
  ...
  $html = Blog::Base::Quiq::JavaScript->script(
      '...'
      '...'
  );
