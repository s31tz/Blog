...
sub roundTo {
    ...
    $x = sprintf '%.*f',$n,$x;
    if ($normalize) {
        $x = Blog::Base::Quiq::Formatter->normalizeNumber($x);
    }
...
Abstand zw. zwei Längengraden (359. und 360.) am Äquator:
  ...
  sprintf '%.2f',Blog::Base::Quiq::Math->geoDistance(0,359,0,360);
  # -> 111.12

Abstand zw. zwei Längengraden am Pol:

  Blog::Base::Quiq::Math->geoDistance(90,359,90,360);
  # -> 0
...
sub valueToPixelX {
    ...
    return Blog::Base::Quiq::Math->roundToInt(($xVal-$xMin)*($width-1)/($xMax-$xMin));
}
...
sub valueToPixelY {
    ...
    return Blog::Base::Quiq::Math->roundToInt($height-1-($yVal-$yMin)*
        ($height-1)/($yMax-$yMin));
}
...
sub valueToPixelYTop {
    my ($this,$height,$yMin,$yMax,$yVal) = @_;
    return Blog::Base::Quiq::Math->roundToInt(($yVal-$yMin)*($height-1)/($yMax-$yMin));
}
