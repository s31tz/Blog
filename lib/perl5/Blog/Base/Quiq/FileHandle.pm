...
Blog::Base::Quiq::FileHandle - Datei-Handle
...
Datei schreiben:
  ...
  my $fh = Blog::Base::Quiq::FileHandle->new('>',$path);
  $fh->print("Test\n");
  $fh->close;
...
Programm darf nur einmal laufen:
  ...
  my $fh = eval{Blog::Base::Quiq::FileHandle->new('<',$file,-lock=>'EXNB')};
  if ($@) {
      exit 5;
  }
...
Zähler-Datei mit Locking:
  ...
  my $fh = Blog::Base::Quiq::FileHandle->new('+>>',$file,-lock=>'EX');
  $fh->seek(0);
  my $count = <$fh> || "0\n";
  chomp $count;
  $fh->truncate;
...
Siehe auch Blog::Base::Quiq::LockedCounter.
...
Filehandle-Objekt für STDOUT:
  ...
  $fh = Blog::Base::Quiq::FileHandle->new(\*STDOUT);
...
sub new {
    ...
    if (ref $_[0] eq 'GLOB') {      # GLOB-Referenz
        ...
        $path = shift;
        if (!ref $path) {
            $path = Blog::Base::Quiq::Path->expandTilde($path);
        }
    }
    ...
    if ($mode) {
        if ($mode eq '<' && (!$path || $path eq '-')) {
            ...
            if ($mode eq '>' && $createDir) {
                Blog::Base::Quiq::Path->mkdir($path,-createParent=>1);
            }
...
Liefere Chunks von 1000 Pfaden:
  ...
  my $fh = Blog::Base::Quiq::FileHandle->new('-|',"find @$dirA -name '*.xml.gz'");
...
Lies gesamten Inhalt einer Datei ab Position $pos:
  ...
  my $fh = Blog::Base::Quiq::FileHandle->open('<',$logFile);
  $fh->seek($pos);
  my $data = $fh->slurp;
...
sub print {
    my $self = shift;
    # @_: @data
    Blog::Base::Quiq::Perl->print($self,@_);
    return;
}
...
sub writeData {
    my ($self,$str) = @_;
    Blog::Base::Quiq::Perl->print($self,pack('I',bytes::length($str)),$str);
    return;
}
