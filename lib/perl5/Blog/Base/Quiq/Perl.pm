...
=head4 Example
  ...
  Blog::Base::Quiq::Perl->autoFlush(*STDOUT);
...
=head4 Example
  ...
  Blog::Base::Quiq::Perl->binmode(*STDOUT,':encoding(utf-8)');
...
=head4 Synopsis
  ...
  Blog::Base::Quiq::Perl->print($fh,@data);
...
=head4 Example
  ...
  Blog::Base::Quiq::Perl->print($fh,"Hello world\n");
...
=head4 Synopsis
  ...
  @arr|$val = Blog::Base::Quiq::Perl->perlDoFile($file);
...
Laden einer Konfigurationsdatei:
  ...
  %cfg = Blog::Base::Quiq::Perl->perlDoFile($file);
...
Liste aller Packages, die das Programm aktuell geladen hat:
  ...
  @arr = Blog::Base::Quiq::Perl->packages;

=item *

Liste in sortierter Form

  @arr = Blog::Base::Quiq::Perl->packages->sort;

=item *

Liste, eingeschränkt auf Packages, deren Name einen Regex erfüllt:

  @arr = Blog::Base::Quiq::Perl->packages->select(qr/patch\d+/);

=item *

Liste aller Packages unterhalb und einschließlich Package X:

  @arr = Blog::Base::Quiq::Perl->packages('X');
...
sub packages {
    ...
    return wantarray? @arr: Blog::Base::Quiq::Array->new(\@arr);
}
...
=head4 Example
  ...
  Blog::Base::Quiq::Perl->classExists('Blog::Base::Quiq::Object');
  ==>
  1
...
=head4 Example
  ...
  Blog::Base::Quiq::Perl->loadClass('My::Application');
...
Der Aufruf Blog::Base::Quiq::Perl->baseClasses('Pkg5') liefert ein Array
mit den Elementen
...
Der Aufruf Blog::Base::Quiq::Perl->baseClassesISA('Pkg5') liefert ein Array
mit den Elementen
...
sub baseClassesISA {
    ...
    my (@arr,%seen);
    for (Blog::Base::Quiq::Perl->hierarchyISA($class)) {
        push @arr,$_ if !$seen{$_}++;
    }
...
Der Aufruf Blog::Base::Quiq::Perl->hierarchyISA('Pkg5') liefert ein Array
mit den Elementen
...
Der Aufruf Blog::Base::Quiq::Perl->subClasses('Pkg1') liefert ein Array
mit den Elementen:
...
Liste in sortierter Form
  ...
  @arr = Blog::Base::Quiq::Perl->subClasses('Pkg1')->sort;

=item *

Liste, eingeschränkt auf Klassen, deren Name einen Regex erfüllt:

  @arr = Blog::Base::Quiq::Perl->subClasses('Pkg1')->select(qr/[45]/);
...
sub subClasses {
    ...
    return wantarray? @arr: Blog::Base::Quiq::Array->bless(\@arr);
}
...
Alias für Subroutine aus anderer Klasse:
  ...
  Blog::Base::Quiq::Perl->createAlias('MyClass',mySub=>\&MyClass1::mySub1);

=item *

Eintrag einer Closure in die Symboltabelle:

  Blog::Base::Quiq::Perl->createAlias(__PACKAGE__,mySub=>sub { <code> });
...
=head4 Example
  ...
  $ref = Blog::Base::Quiq::Perl->getHash($package,'H');
...
Setze Paket-Hash 'h' auf den Wert %hash:
  ...
  $ref = Blog::Base::Quiq::Perl->setHash($package,'h',\%hash);
...
Erzeuge in $class das Array @A:
  ...
  $ref = Blog::Base::Quiq::Perl->createArray($class,'A');

=item *

die Referenz kann geblesst werden:

  bless $ref,'Blog::Base::Quiq::Array';
...
=head4 Example
  ...
  $ref = Blog::Base::Quiq::Perl->getArray($package,'A');
...
Setze Paket-Array 'a' auf den Wert @arr:
  ...
  $ref = Blog::Base::Quiq::Perl->setArray($package,'a',\@arr);
...
Setze Paket-Skalar 'n' auf den Wert 99:
  ...
  $ref = Blog::Base::Quiq::Perl->setScalarValue($package,n=>99);
...
Ermittele Wert von Paket-Skalar 'n':
  ...
  $val = Blog::Base::Quiq::Perl->getScalarValue($package,'n');
...
Skalar
  ...
  $ref = Blog::Base::Quiq::Perl->setVar($package,'$','s',\99);

=item *

Array

  $ref = Blog::Base::Quiq::Perl->setVar($package,'@','a',\@arr);

=item *

Hash

  $ref = Blog::Base::Quiq::Perl->setVar($package,'%','h',\%hash);
...
sub setVar {
    ...
    my $type = Blog::Base::Quiq::Perl->sigilToType($sigil);
...
Definition:
  ...
  $ref = Blog::Base::Quiq::Perl->setSubroutine('My::Class',m=>sub {...});
...
=head4 Example
  ...
  Blog::Base::Quiq::Perl->basicIncPaths;
  ==>
  /etc/perl
