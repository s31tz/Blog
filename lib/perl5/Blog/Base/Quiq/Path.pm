...
use Blog::Base::Quiq::TempFile;
use Blog::Base::Quiq::Shell;
use Blog::Base::Quiq::Terminal;
use Encode::Guess ();
...
sub copy {
    ...
    my $fh1 = Blog::Base::Quiq::FileHandle->new('<',$srcPath);
    ...
    my $fh2 = Blog::Base::Quiq::FileHandle->new('>',$destPath);
    while (<$fh1>) {
...
sub edit {
    ...
    my $encoding = 'utf-8';
    my $changed = 0;
    my $tmpFile = Blog::Base::Quiq::TempFile->new;
    ...
    Blog::Base::Quiq::Shell->exec("$editor $tmpFile");
    ...
    if ($ask) {
        ...
        my $answ = Blog::Base::Quiq::Terminal->askUser(
            "Confirm changes?",
            -values => 'y/n',
            -default => 'y',
        );
...
=head4 Example
  ...
  local $/ = Blog::Base::Quiq::Path->newlineStr($file);
...
Es liegt noch keine Datei vor:
  ...
  $file = Blog::Base::Quiq::Path->nextFile('myfile',3,'log');
  =>
  myfile-001.log
...
Es liegt noch keine Datei mit einer dreistelligen Nummer NNN vor:
  ...
  $num = Blog::Base::Quiq::Path->nextNumber($dir,3);
  =>
  001
...
sub read {
    ...
    if ($decode) {
        ...
        $data = Blog::Base::Quiq::String->autoDecode($data);
    }
...
sub truncate {
    ...
    if ($this->exists($file)) {
        Blog::Base::Quiq::FileHandle->new('>',$file)->truncate;
    }
...
Siehe Blog::Base::Quiq::TempFile->new().
...
sub tempFile {
    my $this = shift;
    return Blog::Base::Quiq::TempFile->new(@_);
}
...
Blog::Base::Quiq::Unindent->hereDoc) und schreibe den uneingerückten Inhalt auf
...
sub unindent {
    ...
    my $data2 = Blog::Base::Quiq::Unindent->hereDoc($data1);
    if ($data1 ne $data2) {
...
Wende Blog::Base::Quiq::Unindent->trimNl() auf die Daten $data an. Dies ist für
...
sub write {
    ...
    if ($encode) {
        Blog::Base::Quiq::Perl->binmode(*F,":encoding($encode)");
    }
    ...
    if (defined($$ref) && $$ref ne '') {
        ...
        if ($unindent) {
            $$ref = Blog::Base::Quiq::Unindent->trimNl($$ref);
        }
...
sub writeInline {
    ...
    $data = Blog::Base::Quiq::Unindent->hereDoc($data);
...
sub count {
    ...
    my $n = 0;
    my $dh = Blog::Base::Quiq::DirHandle->new($dir);
...
sub mkdir {
    ...
    if (@_) {
        Blog::Base::Quiq::Option->extract(-dontExtract=>1,\@_,
...
I<< siehe Blog::Base::Quiq::TempDir >>
...
sub tempDir {
    my $this = shift;
    return Blog::Base::Quiq::TempDir->new(@_);
}
...
Permissions oktal anzeigen
  ...
  printf "%04o\n",Blog::Base::Quiq::Path->mode('/etc/passwd');
  0644
...
sub mtimePaths {
    ...
    my $mtime = Blog::Base::Quiq::Time->new(ymdhms=>$startTime)->epoch;
...
Ausgangslage: Unterhalb von /tmp existieren weder a noch x.
  ...
  Blog::Base::Quiq::Path->write($srcPath,'',-recursive=>1);
  Blog::Base::Quiq::Path->rename($srcPath,$destPath,-recursive=>1);
...
sub rename {
    ...
    if ($recursive) {
        my $newDir = (Blog::Base::Quiq::Path->split($newPath))[0];
        if ($newDir && !-d $newDir) {
    ...
    if ($recursive) {
        while (1) {
            ($oldPath) = Blog::Base::Quiq::Path->split($oldPath);
            eval {Blog::Base::Quiq::Path->rmdir($oldPath)};
            if ($@) {
                last;
            }
        }
...
=head4 Example
  ...
  $ perl -MBlog::Base::Quiq::Path -E 'Blog::Base::Quiq::Path->numberBasePaths([$p->glob("*")],5,10)'
...
=head4 Example
  ...
  $ find SRCDIR -type f | sort | perl -MBlog::Base::Quiq::Path -E 'Blog::Base::Quiq::Path->spreadToSubDirs("link",$destDir,100)'
...
=head4 Example
  ...
  $ perl -MBlog::Base::Quiq::Path -E 'Blog::Base::Quiq::Path->swap("06-album","0000940.*","0010200.*")'
...
=head4 Example
  ...
  Blog::Base::Quiq::Path->symlinkRelative('a','x')
  # x => a
  
  Blog::Base::Quiq::Path->symlinkRelative('a/b','x')
  # x => a/b
  
  Blog::Base::Quiq::Path->symlinkRelative('a/b','x/y')
  # x/y => ../a/b
  
  Blog::Base::Quiq::Path->symlinkRelative('a/b','x/y/z')
  # x/y/z => ../../a/b
...
sub symlinkRelative {
    ...
    if ($createDir) {
        my $dir = (Blog::Base::Quiq::Path->split($symlink))[0];
        if ($dir && !-d $dir) {
            $class->mkdir($dir,-recursive=>1);
        }
    }
    ...
    if ($path !~ m|^/| && $symlink =~ m|/|) {
        if ($symlink !~ m|^/|) {
            ...
            $path = sprintf '%s/%s',Blog::Base::Quiq::Process->cwd,$path;
        }
    }
    if ($verbose) {
        print "$symlink => $path\n";
