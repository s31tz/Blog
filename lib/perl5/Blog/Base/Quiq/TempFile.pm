...
Blog::Base::Quiq::TempFile - Temporäre Datei
...
package Blog::Base::Quiq::TempFile;
...
sub new {
    ...
    my @args;
    if (defined $dir) {
        $dir = Blog::Base::Quiq::Path->expandTilde($dir);
        push @args,'DIR',$dir;
    }
    if (defined $suffix) {
    ...
    if (defined $data) {
        Blog::Base::Quiq::Path->write($self,$data);
    }
    if ($pathOnly) {
        Blog::Base::Quiq::Path->delete("$self");
    }
