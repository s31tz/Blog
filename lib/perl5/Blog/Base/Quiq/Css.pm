...
Blog::Base::Quiq::Css - Generiere CSS Code
...
C<style> eines Blog::Base::Quiq::Html::Tag eine Array-Referenz angegeben
...
Erzeuge Properties für HTML style-Attribut:
  ...
  $properties = Blog::Base::Quiq::Css->properties(
      fontStyle => 'italic',
      marginLeft => '0.5cm',
      marginRight => '0.5cm',
  );
...
Erzeuge eine einfache Style Rule:
  ...
  $rule = Blog::Base::Quiq::Css->rule('p.abstract',
      fontStyle => 'italic',
      marginLeft => '0.5cm',
      marginRight => '0.5cm',
  );
...
B<< Beispiel aus Blog::Base::Quiq::Html::Verbatim >>
...
Angabe) oder gelöscht werden (leere Liste).
  ...
  my $obj = Blog::Base::Quiq::Hash->new(
...
=head4 Synopsis
  ...
  $styleTags = Blog::Base::Quiq::Css->style($h,@specs);
...
B<Code zum Laden eines externen Stylesheet:>
  ...
  $style = Blog::Base::Quiq::Css->style('/css/stylesheet.css');
  =>
  <link rel="stylesheet" type="text/css" href="/css/stylesheet.css" />

B<Stylesheet aus Datei einfügen:>

  $style = Blog::Base::Quiq::Css->style('inline:/css/stylesheet.css');
  =>
  <Inhalt der Datei /css/stylesheet.css>

B<Mehrere Stylesheet-Spezifikationen:>

  $style = Blog::Base::Quiq::Css->style(
      '/css/stylesheet1.css'
      '/css/stylesheet2.css'
  );
  =>
...
sub style {
    ...
    while (@_) {
        ...
        if (ref $spec) {
            ...
            my $data = Blog::Base::Quiq::Path->read($spec);
            ...
            Blog::Base::Quiq::String->removeIndentation(\$spec);
            $style .= "$spec\n";
        }
        else {
            $linkTags .= $h->tag('link',
