...
Blog::Base::Quiq::Image - Operationen im Zusammenhang mit Bildern/Bilddateien
...
Extension wird mittels Blog::Base::Quiq::Image->type() geprüft, ob es sich
...
sub pickImagesWithEog {
    ...
    my $t = Blog::Base::Quiq::Trash->new;
    $t->emptyTrash(1); # Leere Trash nach Rückfrage

    my $fileA = $class->findImages(@_);
    Blog::Base::Quiq::Shell->exec("eog @$fileA 2>/dev/null");
