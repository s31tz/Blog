...
Blog::Base::Quiq::Properties - Eigenschaften einer Menge von skalaren Werten
...
Erzeuge eine formatierte Liste von Float-Werten:
  ...
  my $prp = Blog::Base::Quiq::Properties->new(\@values);
...
sub analyze {
    ...
    my $l;
    if ($val =~ tr/\n//) {
        $l = Blog::Base::Quiq::String->maxLineLength($val);
        $multiLine = 1;
    }
    else {
        $l = length $val;
