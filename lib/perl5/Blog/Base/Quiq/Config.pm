...
Blog::Base::Quiq::Config - Konfigurationsdatei in "Perl Object Notation"
...
=head1 SYNOPSIS
  ...
  my $cfg = Blog::Base::Quiq::Config->new('/etc/myapp/test.conf');
  my $database = $cgf->get('database');

=head1 DESCRIPTION

Ein Objekt der Klasse Blog::Base::Quiq::Config repräsentiert eine Menge von
...
sub new {
    ...
    if ($_[0] =~ /=>/) { # "$key => $val, ..."
    ...
    elsif (Blog::Base::Quiq::Reference->isHashRef($_[0])) { # \%keyVal
        %cfg = %{shift()};
    }
    else {
        # @files
        ...
        if (!defined $cfgFile) {
            if (defined $create) {
                ...
                $create = Blog::Base::Quiq::Unindent->trimNl($create);
        ...
        %cfg = Blog::Base::Quiq::Perl->perlDoFile($cfgFile);
    }
...
sub try {
    ...
    for my $key (@_) {
        ...
        if (!ref $val && defined $val) {
            ...
            for my $key ($val =~ /%(\w+)%/g) {
                if ($key eq 'CWD') {
                    $val =~ s/%CWD%/Blog::Base::Quiq::Process->cwd/e;
                }
                else {
