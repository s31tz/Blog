...
Blog::Base::Quiq::Udl - Universal Database Locator
...
Objekt instantiieren:
  ...
  my $udl = Blog::Base::Quiq::Udl->new($udlStr);

UDL aus Konfigurationsdatei:

  my $udl = Blog::Base::Quiq::Udl->new('test-db');
...
sub new {
    ...
    my $self = $class->SUPER::new(
        ...
        host => '',
        port => '',
        # FIXME: auf Blog::Base::Quiq::Hash::Ordered umstellen
        options => Blog::Base::Quiq::Hash->new->unlockKeys,
    );
    if (@_ == 1 && $_[0] !~ /^[a-z]+#/) {
        ...
        my $udlStr = Blog::Base::Quiq::Database::Config->new->udl(shift);
        $self->udl($udlStr);
    }
    elsif (@_) {
        $self->udl(@_);
...
sub apiClass {
    my $self = shift;
    return 'Blog::Base::Quiq::Database::Api::'.ucfirst($self->api).'::Connection';
}
...
sub dsn {
    ...
    if ($dbms eq 'mysql') {
        ...
        $db = Blog::Base::Quiq::Path->expandTilde($db);
