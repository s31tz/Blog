...
sub DESTROY {
    ...
    if ($self->{'timeSummary'} && $self->{'log'}) {
        ...
        my $duration = Blog::Base::Quiq::Duration->new($t)->asString(2);

        # printf $fd "%s%s: %s\n",$pre,$prog,$duration;
        my $esc = $self->{'cmdAnsiColor'};
        my $a = Blog::Base::Quiq::AnsiColor->new($esc);
...
Programmaufruf um 1 erhöht wird. Beispiel:
  ...
  perl -MBlog::Base::Quiq::Shell -E 'Blog::Base::Quiq::Shell->exec("echo hallo",-outputTo=>"echo")'
...
sub exec {
    ...
    my $qx = 0;
    # FIXME: auf Blog::Base::Quiq::TempFile umstellen
    ...
    unless ($dryRun) {
        ...
        if ($log && $self->{'time'}) {
            my $fd = $self->{'logDest'};
            printf $fd "%s%s\n",$self->{'timePrefix'},
                Blog::Base::Quiq::Converter->epochToDuration($t1-$t0,1,3);
        }
        if ($except) {
...
sub cd {
    my $self = shift;
    my $dir = Blog::Base::Quiq::Path->expandTilde(shift);
    ...
    my $cwd = Blog::Base::Quiq::Process->cwd;
