...
=head4 Synopsis
  ...
  $out = Blog::Base::Quiq::Ipc->filter($cmd,$in,@opt);
  ($out,$err) = Blog::Base::Quiq::Ipc->filter($cmd,$in,@opt);
  $out = Blog::Base::Quiq::Ipc->filter($cmd,@opt);
  ($out,$err) = Blog::Base::Quiq::Ipc->filter($cmd,@opt);
...
werden soll, wenn STDOUT kein Terminal ist:
  ...
  Blog::Base::Quiq::Ipc->pipeTo($data,'less -R',-toStdout=>!-t);
...
sub pipeTo {
    ...
    my $fh = $toStdout? \*STDOUT: Blog::Base::Quiq::FileHandle->new('|-',$cmd);
    if ($encoding) {
