...
Blog::Base::Quiq::Program - Basisklasse für Programme
...
=head4 Synopsis
  ...
  $prg = Blog::Base::Quiq::Program->run($programClass,@options);
...
sub run {
    ...
    my $self = $class->new(@_);
    eval {
        Blog::Base::Quiq::Perl->loadClass($programClass);
        ...
        while (@arr) {
            ...
            if ($key eq 'programClass') {
                ...
                Blog::Base::Quiq::Perl->loadClass($subProgramClass);
...
sub parameters {
    ...
    my ($argA,$opt) = Blog::Base::Quiq::Parameters->extract(0,0,
...
Prüfe Werte durch Methoden der Klasse Blog::Base::Quiq::Assert. Ist eine
Zusicherung verletzt, wird die betreffende Exception in die
Ausgabe der Programm-Hilfeseite umgesetzt. Die Subroutine
erhält als Argument ein instantiiertes Blog::Base::Quiq::Assert-Objekt.
...
sub assert {
    ...
    my $a = Blog::Base::Quiq::Assert->new(
        stacktrace => 0,
...
sub options {
    ...
    my $optH = eval{Blog::Base::Quiq::Option->extract(
        -simpleMessage => 1,
        # -mode => 'sloppy',
        $argA,
        @_
...
sub help {
    ...
    my $fh = Blog::Base::Quiq::FileHandle->new('<',$0);
    while (<$fh>) {
        chomp;
        if (/^=encoding\s+(\S+)/) {
            $podEncoding = $1;
    ...
    if ($exitCode) {
        ...
        my $fh = Blog::Base::Quiq::FileHandle->new('|-',
...
sub new {
    ...
    my $encoding = Blog::Base::Quiq::System->encoding;
    Blog::Base::Quiq::Perl->binmode(*STDIN,":encoding($encoding)");
    Blog::Base::Quiq::Perl->binmode(*STDOUT,":encoding($encoding)");
    Blog::Base::Quiq::Perl->binmode(*STDERR,":encoding($encoding)");
