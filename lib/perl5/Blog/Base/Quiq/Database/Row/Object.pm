...
Blog::Base::Quiq::Database::Row::Object - Datensatz als Objekt

=head1 BASE CLASS

L<Blog::Base::Quiq::Database::Row>
...
package Blog::Base::Quiq::Database::Row::Object;
use base qw/Blog::Base::Quiq::Database::Row/;
...
our $TableClass = 'Blog::Base::Quiq::Database::ResultSet::Object';
...
sub new {
    ...
    if (ref $_[0]) {
        if (Scalar::Util::blessed($_[0]) &&
                $_[0]->isa('Blog::Base::Quiq::Database::Connection')) {
    ...
    my $hash = Blog::Base::Quiq::Hash->new($titleA,$valueA? $valueA: (),'')
        ->unlockKeys;
...
sub getSet {
    ...
    if (@_) {
        ...
        if ($val ne $oldVal) {
            ...
            unless ($self->[3]) {
                $self->[3] = Blog::Base::Quiq::Hash->new->unlockKeys;
            }
...
sub addChildType {
    ...
    return $self->[4]->{$type} =
        Blog::Base::Quiq::Database::ResultSet->new($rowClass,$titleA);
}
...
Basisklasse Blog::Base::Quiq::Database::Row. Die Methoden hat die Klasse mit
der Klasse Blog::Base::Quiq::Database::Row::Array gemeinsam.
...
sub asRecord {
    ...
    my $a = $color // Blog::Base::Quiq::AnsiColor->new(1);
    $indent = ' ' x $indent;
...
sub copyData {
    ...
    if ($ignore) {
        $ignore = Blog::Base::Quiq::Hash->new($ignore,1)->unlockKeys;
    }
