...
Blog::Base::Quiq::Database::ResultSet::Object - Liste von Datensätzen in Objekt-Repräsentation
...
sub normalizeNumber {
    ...
    for my $row (@{$self->rows}) {
        for my $title (@_) {
            my $val = $row->$title;
            $val = Blog::Base::Quiq::Formatter->normalizeNumber($val);
            $row->$title($val);
        }
    }
...
Tabellenobjekt (Blog::Base::Quiq::Database::ResultSet::Object)
