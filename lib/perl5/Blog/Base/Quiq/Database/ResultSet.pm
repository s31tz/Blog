...
Liefere einen Hash bzw. eine Hashreferenz (Blog::Base::Quiq::Hash) mit den
...
sub formats {
    ...
    if (!$self->{'formatA'} || $force) {
        ...
        for (my $i = 0; $i < @$titleA; $i++) {
            ...
            my $prp = Blog::Base::Quiq::Properties->new;
            for my $row (@$rowA) {
...
Liefere den Namen der Default-Rowklasse: 'Blog::Base::Quiq::Database::Row::Object'
...
Als Default-Rowklasse wird für die Blog::Base::Quiq::Database::ResultSet-
Klassenhierarchie 'Blog::Base::Quiq::Database::Row::Object' definiert.

Die Methode wird in der Subklasse Blog::Base::Quiq::Database::ResultSet::Array
überschrieben. Für diesen Zweig ist die Default-Rowklasse
'Blog::Base::Quiq::Database::Row::Array'.
...
sub defaultRowClass {
    return 'Blog::Base::Quiq::Database::Row::Object';
}
...
sub asTable {
    ...
    if ($self->count) {
        ...
        my $asArray = $self->isRaw || Blog::Base::Quiq::Array->eq(
    ...
    if ($info) {
        ...
        if (my $duration = $self->execTime + $self->fetchTime) {
            $tmp .= ', '.Blog::Base::Quiq::Duration->new($duration)->asShortString(
                -precision => 3,
            );
        }
