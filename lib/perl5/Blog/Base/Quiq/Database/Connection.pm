...
use Blog::Base::Quiq::Sql;
use Blog::Base::Quiq::Object;
use Blog::Base::Quiq::Option;
use Blog::Base::Quiq::Udl;
...
=item -sqlClass => $class (Default: 'Blog::Base::Quiq::Sql')
...
sub new {
    my ($class,$self) = Blog::Base::Quiq::Object->this(shift);
    ...
    $sqlClass = 'Blog::Base::Quiq::Sql' if !defined $sqlClass;
    ...
    my $udlObj = ref $udl? $udl: Blog::Base::Quiq::Udl->new($udl);
    my $apiObj = Blog::Base::Quiq::Database::Api->connect($udlObj,
        -autoCommit => $autoCommit,
        -handle => $handle,
        -utf8 => $utf8,
    );
    ...
    $self = $class->SUPER::new(
        ...
        sqlClass => $sqlClass,
        sqlObj => $sqlObj,
        titleListCache => Blog::Base::Quiq::Hash->new->unlockKeys,
        nullRowCache => Blog::Base::Quiq::Hash->new->unlockKeys,
        utf8 => $utf8,
        schema => undef,
        log => $log,
        logfile => $logfile,
...
=head3 newFromSbit() - Instantiiere Blog::Base::Quiq::Database::Connection-Datenbankobjekt aus Sbit-Datenbankobjekt
...
=head4 Example
  ...
  my $db = Blog::Base::Quiq::Database::Connection->new('dbi#mysql',
      -handle => $main::dbh,
  );
...
Liefere den Namen der Default-Rowklasse:
  ...
  Blog::Base::Quiq::Database::Row::Object  ($raw ist "falsch")
  Blog::Base::Quiq::Database::Row::Array   ($raw ist "wahr")
...
sub defaultRowClass {
    return $_[1]? 'Blog::Base::Quiq::Database::Row::Array':
        'Blog::Base::Quiq::Database::Row::Object';
}
...
Für die gesamte Session:
  ...
  $db = Blog::Base::Quiq::Database::Connection->new($udl,-log=>1);
...
sub writeLog {
    ...
    if ($self->{'log'}) {
        ...
        if ($file eq '-') {
            ...
            $fh = Blog::Base::Quiq::FileHandle->new('>>',$file);
        }
        print $fh @_;
    }
...
sub exportTable {
    ...
    my $fh = Blog::Base::Quiq::FileHandle->new('>',$file);
    $fh->setEncoding('utf-8');
    $fh->print(Blog::Base::Quiq::Array->dump($titleA,$colSep),"\n");
    while (my $rowA = $cur->fetch) {
        $fh->print(Blog::Base::Quiq::Array->dump($rowA,$colSep),"\n");
    }
    $fh->close;
    $cur->close;
...
sub importTable {
    ...
    my $line = scalar <$fh>;
    chomp $line;
    my $titleA = Blog::Base::Quiq::Array->restore($line,$colSep);
    ...
    while (my $line = <$fh>) {
        chomp $line;
        $cur->bind(Blog::Base::Quiq::Array->restore($line,$colSep));
    }
    $fh->close;
    $cur->close;
...
Row-Objekte. Als Default-Rowklasse verwende Blog::Base::Quiq::Database::Row::Array
statt Blog::Base::Quiq::Database::Row::Object (der Parameter -rowClass überschreibt
diesen Default).

=item -rowClass => $rowClass (Default: 'Blog::Base::Quiq::Database::Row::Object')
...
Bei Raw-Datensätzen ist Blog::Base::Quiq::Database::ResultSet::Array der Default,
ansonsten Blog::Base::Quiq::Database::ResultSet::Object.

=back

=head4 Returns

Referenz auf Cursor-Objekt (Blog::Base::Quiq::Database::Cursor)
...
sub sql {
    ...
    Blog::Base::Quiq::String->removeIndentation(\$stmt);
    ...
    if (defined($cache) && $stmt) {
        ...
        $p->mkdir($cacheDir);
        $cacheFile = $cacheDir.'/'.Blog::Base::Quiq::Digest->md5($stmt);
    ...
    if ($cacheOp eq 'w') {
        ...
        $cacheFh = Blog::Base::Quiq::FileHandle->new('>',$cacheFile,-createDir=>1);
        my $width = @$titles;
        ...
        $cacheFh = Blog::Base::Quiq::FileHandle->new('<',$cacheFile);
    ...
    return Blog::Base::Quiq::Database::Cursor->new(
        apiCur => $apiCur,
        bindVars => $bindVars,
        cacheFile => $cacheFile,
        cacheFh => $cacheFh,
...
sub execute {
    ...
    if ($cur->isSelect) {
        ...
        my $a = Blog::Base::Quiq::AnsiColor->new($color);
...
sub patchMethods {
    ...
    my @arr;
    my $refH = Blog::Base::Quiq::Perl->stash($patchClass);
    for my $name (sort keys %$refH) {
        if ($name =~ /^patch\d+/) {
            my $descr = Blog::Base::Quiq::Perl->getScalarValue($patchClass,$name);
...
# FIXME: Methode in Blog::Base::Quiq::Database::Row::Object::Table auf diese zurückführen?
...
Siehe Blog::Base::Quiq::Sql->select()
...
Siehe Blog::Base::Quiq::Database::Connection/sql().
...
sub selectFrom {
    my $self = shift;
    my $stmt = Blog::Base::Quiq::Unindent->trim(shift);
    # @_: @select,@opt
...
=item -rowClass => $class (Default: 'Blog::Base::Quiq::Database::Row::Object')

Default Datensatz-Klasse. Im Falle von -raw=>1 ist
'Blog::Base::Quiq::Database::Row::Array' der Default.
...
Null-Datensatz einer Tabelle instantiieren
  ...
  $per = Blog::Base::Quiq::Database::Connection->nullRow('person');
...
Im Skalarkontext wird ein Objekt der Klasse C<< Blog::Base::Quiq::Array >> oder
der Klasse C<< Blog::Base::Quiq::Hash >> geliefert. Letzteres, wenn Option
...
sub values {
    ...
    if (wantarray) {
        ...
        return Blog::Base::Quiq::Hash->new({@arr})->unlockKeys;
    }
    else {
        return Blog::Base::Quiq::Array->new(\@arr);
    }
}
...
sub schemas {
    ...
    if ($hash) {
        return Blog::Base::Quiq::Array->toHash($schemaA);
    }
...
sub copyData {
    ...
    if ($srcDb->tableExists($srcTable)) {
        ...
        my $srcTitleH = Blog::Base::Quiq::Hash->new(\@srcTitles,1)->unlockKeys;
...
Siehe Blog::Base::Quiq::Sql::addCheckConstraint()
...
Siehe Blog::Base::Quiq::Sql::addForeignKeyConstraint()
...
sub tableDiff {
    ...
    return Blog::Base::Quiq::Database::ResultSet->new(\@columns,\@rows);
}
...
Ergebnismengen-Objekt (Blog::Base::Quiq::ResultSet::Object)
...
sub updateNToM {
    ...
    Blog::Base::Quiq::Parameters->extractPropertiesToVariables(\@_,
        a => \$a,
        b => \$b,
        bPk => \$b_pk,
        bCol => \$b_col,
    ...
    my ($only1A,$only2A) = Blog::Base::Quiq::Array->different($values1A,$values2A);
...
Dies ist ein Objekt der Klasse Blog::Base::Quiq::Sql, das beim
Verbindungsaufbau passend zum DBMS instantiiert wurde.

Alle SQL-Generierungsmethoden der Klasse Blog::Base::Quiq::Sql können
...
Die meisten Statements der Klasse Blog::Base::Quiq::Sql können auch
...
und Ausführung vorzuziehen, da die Blog::Base::Quiq::Database::Connection-Methoden bei der
Ausführung teilweise DBMS-abhängige Sonderbehandlungen vornehmen.

=head2 Erweiterte Statement-Generierung

Anstelle der Default Sql-Klasse Blog::Base::Quiq::Sql kann beim Verbindungsaufbau
eine anwendungspezifische Klasse vereinbart werden:

  package MyApp::Sql;
  use base qw/Blog::Base::Quiq::Sql/;
  
  ...
  
  package main;
  
  $db = Blog::Base::Quiq::Database::Connection->new(...,-sqlClass=>'MyApp::Sql');
...
=head3 Daten speichern
  ...
  my $foto = Blog::Base::Quiq::Path->read('/home/pirelli/Picture/elli.jpg');
...
UTF-8 Encoding verwendet:
  ...
  $db = Blog::Base::Quiq::Database::Connection->new($udl,-utf8=>1);
...
Beispiel: Eine DBI MySQL-Handle $dbh wird als Lowlevel-Handle verwendet,
  ...
  UDL braucht nur die
     ...
     $db = Blog::Base::Quiq::Database::Connection->new('dbi#mysql',-handle=>$dbh);
