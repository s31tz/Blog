...
Blog::Base::Quiq::Database::Config - Datenbank-Konfiguration

=head1 BASE CLASS

L<Blog::Base::Quiq::Config>
...
ungelockten Blog::Base::Quiq::Hash instantiiert. Entsprechend können die
Methoden der Klasse Blog::Base::Quiq::Hash zum Lesen der
...
sub new {
    ...
    if (!Blog::Base::Quiq::Path->exists($file) && $sloppy) {
        return undef;
    }
    ...
    for (keys %$self) {
        $self->{$_} = Blog::Base::Quiq::Hash->new($self->{$_})->unlockKeys;
    }
