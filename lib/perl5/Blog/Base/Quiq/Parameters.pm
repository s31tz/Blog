...
Optionen eine Exception wirft:
  ...
  sub parameters {
      ...
      my ($argA,$opt) = Blog::Base::Quiq::Parameters->extract($varMode,$properties,
...
sub extract {
    ...
    while ($i < @$paramA) {
        ...
        if (!$properties && ($noMoreOptions || !defined($param) ||
            ...
            $key = Blog::Base::Quiq::Converter->snakeCaseToCamelCase($key);
    ...
    return $varMode? \@args: (\@args,Blog::Base::Quiq::Hash->new(\%opt));
}
...
werden als Property/Wert-Paare übergeben:
  ...
  sub table {
      ...
      my $opt = Blog::Base::Quiq::Parameters->extractPropertiesToObject(\@_,
          alignments => [],
