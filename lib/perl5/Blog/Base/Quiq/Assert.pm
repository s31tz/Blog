...
Blog::Base::Quiq::Assert - Zusicherungen
...
Test von der Kommandozeile aus:
  ...
  $ perl -MBlog::Base::Quiq::Assert -E 'Blog::Base::Quiq::Assert->new->isNotNull("",-name=>"x")'
  Exception:
...
sub pathExists {
    ...
    if (!defined($path) || $path eq '' || Blog::Base::Quiq::Path->exists($path)) {
        return 1;
    }
    elsif (defined wantarray) {
        return 0;
...
sub isDate {
    ...
    if (defined($val) && $val ne '') {
        local $@;
        eval {Blog::Base::Quiq::Time->new(split /-/,$val)};
        if ($@) {
...
sub isNumber {
    ...
    if (!defined($val) || $val eq '' || Blog::Base::Quiq::Math->isNumber($val)) {
        return 1;
    }
    elsif (defined wantarray) {
        return 0;
