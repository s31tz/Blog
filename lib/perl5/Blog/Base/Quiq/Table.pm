...
Mit Kolumnennamen:
  ...
  $tab = Blog::Base::Quiq::Table->new(['a','b','c'],\@rows);
...
Ohne Kolumnennamen:
  ...
  # Objekt instantiieren
  $tab = Blog::Base::Quiq::Table->new(3,\@rows);
...
sub new {
    ...
    if (ref $arg) {
        ...
        $self->set(
            width => scalar @$arg,
            columnA => $arg,
            columnH => Blog::Base::Quiq::Hash->new({map {$_ => $i++} @$arg}),
        );
    }
    else {
        # $width
...
Properties-Objekt (Blog::Base::Quiq::Properties)
...
sub properties {
    ...
    if (!$prp) {
        $prp = Blog::Base::Quiq::Properties->new;
...
sub push {
    ...
    my $row = Blog::Base::Quiq::TableRow->new($self,$valueA);
...
Liste der Zeilen (Objekte der Klasse Blog::Base::Quiq::TableRow). Im Skalarkontext
...
Liefere die Tabelle in der Repräsentation der Klasse Blog::Base::Quiq::AsciiTable.

=head4 Example

  $tab = Blog::Base::Quiq::Table->new(['Integer','String','Float'],[
      [1,  'A',  76.253],
      [12, 'AB', 1.7   ],
      [123,'ABC',9999  ],
  ]);
...
Die Terminal-Farbe ist eine Zeichenkette, wie sie Blog::Base::Quiq::AnsiColor
...
=head4 Example
  ...
  $tab = Blog::Base::Quiq::Table->new(3,[
      [1,  'A',  76.253],
      [12, 'AB', 1.7   ],
      [123,'ABC',9999  ],
  ]);
...
sub asText {
    ...
    # Terminal-Farben
    my $a = Blog::Base::Quiq::AnsiColor->new;
