...
Blog::Base::Quiq::Json::Code - Erzeuge JSON-Code in Perl
...
package Blog::Base::Quiq::Json::Code;
...
sub code {
    my ($self,$text) = @_;
    my $code = Blog::Base::Quiq::Unindent->trim($text);
...
Schlüssel aus dem Zeichenvorrat eines JavaScript-Bezeichners:
  ...
  $str = $j->Blog::Base::Quiq::Json::Code('borderWidth');
  ==>
  "borderWidth"

Schlüssel mit Zeichen, die nicht in einem JavaScript-Bezeichner vorkommen:

  $str = $j->Blog::Base::Quiq::Json::Code('border-width');
  ==>
  "'border-width'"
