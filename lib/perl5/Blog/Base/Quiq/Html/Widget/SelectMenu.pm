...
Blog::Base::Quiq::Html::Widget::SelectMenu - Liste mit Einzelauswahl
...
sub html {
    ...
    if ($readonly) {
        ...
        $html .= Blog::Base::Quiq::Html::Widget::TextField->html($h,
            # name => $name,
            # id => $id,
            class => $class,
            style => $style,
        ...
        $html = $h->tag('select',
            ...
            style => $style,
            disabled => $disabled,
            onchange => Blog::Base::Quiq::JavaScript->line($onChange),
            title => $title,
            '-',
            $str
        );
