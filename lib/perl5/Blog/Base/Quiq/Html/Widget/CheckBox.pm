...
Blog::Base::Quiq::Html::Widget::CheckBox - Checkbox

=head1 BASE CLASS

L<Blog::Base::Quiq::Html::Widget>
...
package Blog::Base::Quiq::Html::Widget::CheckBox;
...
use Blog::Base::Quiq::Html::Widget::Hidden;
...
sub html {
    ...
    if (ref $option) { # option => [$checkedValue,$uncheckedValue]
        $html .= Blog::Base::Quiq::Html::Widget::Hidden->html($h,
            name => $name,
        ...
        my $js = 'this.previousElementSibling.value = this.checked?'.
            " '$option->[0]': '$option->[1]'";
        if (my $onClick = Blog::Base::Quiq::JavaScript->line($onClick)) {
           $js .= "; $onClick";
        }
        ...
        $html .= $h->tag('input',
            ...
            title => $title,
            onclick => Blog::Base::Quiq::JavaScript->line($onClick),
        );
    }
