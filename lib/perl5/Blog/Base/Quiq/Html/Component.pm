...
Blog::Base::Quiq::Html::Component - Eigenständige Komponente einer HTML-Seite
...
=head1 SYNOPSIS
  ...
  $c = Blog::Base::Quiq::Html::Component->new(
      name => $name
      resources => \@resources,
      css => $css | \@css,
      html => $html | \@html,
...
Blog::Base::Quiq::Html::Component::Bundle. Ein Objekt der Klasse
...
Bündel von Komponenten ausgewählt werden. Siehe Blog::Base::Quiq::Html::Bundle.
...
sub fragment {
    ...
    return $h->cat(
        ...
        Blog::Base::Quiq::Css->style($h,$cssA),
        join('',@$htmlA),
        Blog::Base::Quiq::JavaScript->script($h,$jsA),
        $h->tag('script',
            -ignoreIfNull => 1,
            Blog::Base::Quiq::JQuery::Function->ready(join('',@$readyA))
        ),
    );
}
