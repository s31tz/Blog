...
Blog::Base::Quiq::Hash - Zugriffssicherer Hash mit automatisch generierten Attributmethoden
...
Hash-Objekt instantiieren:
  ...
  my $h = Blog::Base::Quiq::Hash->new(a=>1,b=>1,c=>3);
...
sub AUTOLOAD :lvalue {
    ...
    *{$AUTOLOAD} = sub :lvalue {
        ...
        # if (!exists $self->{$key}) {
        #     die Blog::Base::Quiq::Stacktrace->asString;
        # }
...
entsperrt werden:
  ...
  return Blog::Base::Quiq::Hash->new(...)->unlockKeys;
