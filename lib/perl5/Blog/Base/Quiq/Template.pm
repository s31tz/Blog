...
Blog::Base::Quiq::Template - Klasse für HTML/XML/Text-Generierung
...
Code:
  ...
  $tpl = Blog::Base::Quiq::Template->new('html','test.html');
  -oder-
  $tpl = Blog::Base::Quiq::Template->new('html',\$str);
...
=head4 Synopsis
  ...
  $tpl = Blog::Base::Quiq::Template->new($type,$file,@opt);
  $tpl = Blog::Base::Quiq::Template->new($type,\$str,@opt);
...
sub new {
    ...
    my $str = ref $arg? $$arg: Blog::Base::Quiq::Path->read($arg);
...
=head4 Example
  ...
  $js = Blog::Base::Quiq::Template->combine(
      placeholders => [
          __NAME__ => $name,
          __CONFIG__ => $config,
      ],
...
sub combine {
    ...
    $template = Blog::Base::Quiq::Unindent->string('~',$template);
...
Subroutine liefert Platzhalter-Wert:
  ...
  my $tpl = Blog::Base::Quiq::Template->new('xml',\$Order);
  $tpl->replace(
      __CUSTNR__ => $kundenNr,
      ...
      __ORDERLINES__ => sub {
          ...
          my $i = 0;
          for my $pos (@$posA) {
              my $tpl = Blog::Base::Quiq::Template->new('xml',\$OrderLine);
              $tpl->replace(
                  __I__ => $i++,
...
sub key {
    ...
    if (ref $arg) {
        if (Blog::Base::Quiq::Reference->isArrayRef($arg)) {
            return $arg->[0];
        }
        else {
            return $$arg;
...
sub value {
    ...
    elsif (Scalar::Util::blessed($arg) && $arg->isa('Blog::Base::Quiq::Template')) {
        # Referenz auf Template-Objekt -> asString() aufrufen
        return $arg->asString;
    }
    elsif (Blog::Base::Quiq::Reference->isArrayRef($arg)) {
        # Array-Referenz -> Werte mit "\n" konkatenieren
        return join "\n",map {$self->value($_)} @$arg;
    }
    elsif (Blog::Base::Quiq::Reference->isCodeRef($arg)) {
