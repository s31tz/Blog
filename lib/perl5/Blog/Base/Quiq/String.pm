...
Blog::Base::Quiq::String - Operationen auf Zeichenketten
...
sub determineIndentation {
    ...
    return Blog::Base::Quiq::Array->gcd([keys %ind]);
}
...
Reduktion auf Einrücktiefe 2:
  ...
  Blog::Base::Quiq::String->reduceIndentation(2,$str);
...
HTML, XML:
  ...
  $code = Blog::Base::Quiq::String->removeComments($code,'<!--','-->');
...
Verwende die Tilde (~) als Fortsetzungszeichen:
  ...
  Blog::Base::Quiq::String->joinLines('~',"Dies ist ~\nein Text");
  ==>
  "Dies ist ein Text"
...
Maximale Zeilenlänge auf 12 Zeichen begrenzen:
  ...
  $txt = Blog::Base::Quiq::String->wrap($txt,-width=>12);
  # =>
  Dies ist ein
  Test mit
  einem kurzen
