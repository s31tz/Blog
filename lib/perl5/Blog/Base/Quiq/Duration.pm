...
Blog::Base::Quiq::Duration - Rechnen und Konvertieren von Zeiträumen
...
=head4 Synopsis
  ...
  $dur = Blog::Base::Quiq::Duration->new($sec);
  $dur = Blog::Base::Quiq::Duration->new($str);
...
sub asShortString {
    ...
    if ($minUnit =~ /^[dhm]$/ && $arr[3]) {
        $arr[2] += Blog::Base::Quiq::Math->roundToInt($arr[3]/60);
        $arr[3] = 0;
    }
    if ($minUnit =~ /^[dh]$/ && $arr[2]) {
        $arr[1] += Blog::Base::Quiq::Math->roundToInt($arr[2]/60);
        $arr[2] = 0;
    }
    if ($minUnit eq 'd' && $arr[1]) {
        $arr[0] += Blog::Base::Quiq::Math->roundToInt($arr[1]/24);
        $arr[1] = 0;
    }
...
Zeitdauer-Zeichenkette bestehend aus Tagen, Stunden, Mintuten, Sekunden:
  ...
  $sec = Blog::Base::Quiq::Duration->stringToSeconds('152d5h25m3.457s');
  # 13152303.457

Dasselbe mit Doppelpunkt-Notation:

  $sec = Blog::Base::Quiq::Duration->stringToSeconds('152:5:25:3.457');
  # 13152303.457
