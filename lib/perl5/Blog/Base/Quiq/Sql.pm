...
Blog::Base::Quiq::Sql - Klasse zur Generierung von SQL

=head1 BASE CLASS

L<Blog::Base::Quiq::Dbms>
...
Das Programm
  ...
  my $sql = Blog::Base::Quiq::Sql->new('Oracle');
...
So verhält es sich im Detail:
  ...
  $script = Blog::Base::Quiq::Sql->stmtListToScript(
      '-- TEXT1',
      'STMT1',
      "STMT2\n...',
      "STMT3 (\n....\n)",
...
für das DBMS angegeben ist:
  ...
  $sql = Blog::Base::Quiq::Sql->new('Oracle');
  $type = $sql->columnDef(
      type => 'STRING(20)',
  );
  ==>
...
sub columnDef {
    ...
    my $keyVal = Blog::Base::Quiq::Hash->new(
        default => undef,
        null => undef,
        notNull => undef,
        primaryKey => undef,
...
sub createFunction {
    ...
    my $signature = shift;
    my $body = Blog::Base::Quiq::String->removeIndentation(shift);
...
sub select {
    ...
    Blog::Base::Quiq::Option->extractMulti(\@_,
        -comment => \$comment,
        -with => \@with,
        -select => \@select,
        -distinct => \$distinct,
    ...
    if (@placeholders) {
        $stmt = Blog::Base::Quiq::Template->combine(
            placeholders => \@placeholders,
            template => $stmt,
        );
    }
...
sub insert {
    ...
    if (ref $_[0]) {
        if (Scalar::Util::blessed($_[0]) &&
                $_[0]->isa('Blog::Base::Quiq::Database::Row::Object')) { # $row
            my $row = shift;
...
sub insertMulti {
    ...
    my $sql = sprintf Blog::Base::Quiq::Unindent->string('
        INSERT INTO %s
            (%s)
        VALUES
    '),$table,join(', ',@$keyA);
...
sub opIN {
    ...
    if (@_ == 1 && $_[0] =~ /SELECT/i) {
        # Subselect
        $str = Blog::Base::Quiq::String->removeIndentationNl($_[0]);
        $str =~ s/^/        /mg;
        return "$op (\n$str    )";
    }
...
sub whereExpr {
    ...
    elsif (Blog::Base::Quiq::Reference->isArrayRef($expr)) {
        return $opExpr;
    }
    else {
        return "= $opExpr";
...
sub fromClause {
    ...
    for my $expr (@from) {
        ...
        $expr = Blog::Base::Quiq::String->removeIndentation($expr);
        # $expr =~ s/\n/\n    /g;
    }
...
sub whereClause {
    ...
    while (@_) {
        if (defined($_[0]) && $_[0] =~ /[\s=><]/) {
            ...
            my $clause = Blog::Base::Quiq::String->removeIndentation(shift);
            if ($clause =~ /\n/) {
...
sub union {
    ...
    for (@_) {
        ...
        $stmt .= Blog::Base::Quiq::Unindent->trim($_);
    }
...
sub diff {
    ...
    my @opt = Blog::Base::Quiq::Option->extractAll(\@_);
    my @where = (@_,'('.join(" OR\n    ",@or).')');
