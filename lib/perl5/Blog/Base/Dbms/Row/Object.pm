package Blog::Base::Dbms::Row::Object;
use base qw/Blog::Base::Dbms::Row/;
...
Blog::Base::Dbms::Row::Object - Datensatz als Objekt
...
our $TableClass = 'Blog::Base::Dbms::ResultSet::Object'; # Default-Tabellenklasse
...
sub new {
    ...
    if (ref $_[0]) {
        if (Scalar::Util::blessed($_[0]) &&
                $_[0]->isa('Blog::Base::Dbms::Database')) {
    ...
    if (ref $titleA eq 'ARRAY') {
        $titleA = bless $titleA,'Blog::Base::Array';
    }
...
=head3 parent() - Liefere Eltern-Datensatz

=head4 Synopsis

    $parentRow = $row->parent($type);
...
=head3 childs() - Liefere Kind-Datensätze

=head4 Synopsis

    @rows|$rowT = $row->childs($type);
...
Basisklasse Blog::Base::Dbms::Row. Die Methoden hat die Klasse mit
der Klasse Blog::Base:Dbms::Row::Array gemeinsam.
...
Die Methode ist nützlich, wenn über die Blog::Base::Misc-Klassenbibliothek
