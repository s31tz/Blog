...
Blog::Base::TextFile - Textdatei als Array von Zeilen
...
=item -lineClass => $class (Default: 'Blog::Base::TextFile::Line')
...
sub new {
    ...
    my $lineClass = 'Blog::Base::TextFile::Line';
    ...
    if (ref($inp) eq 'ARRAY') { # Zeilen in neues Dokument
        ...
        my $fh = Blog::Base::FileHandle->new('<',$inp);
        while (<$fh>) {
            chomp;
