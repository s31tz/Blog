...
Blog::Base::Sdoc::Node - Basisklasse für die Knoten eines Sdoc-Dokuments (abstrakt)
...
sub nextType {
    ...
    if ($type eq 'Item') {
        ...
        if (!$self->isa('Blog::Base::Sdoc::List') ||
        ...
        if (!$self->isa('Blog::Base::Sdoc::Table')) {
            $type = 'Table';
        }
    }
        ...
        if (!$self->isa('Blog::Base::Sdoc::KeyValTable')) {
            $type = 'KeyValTable';
        }
    }
...
sub parseObjectSpec {
    ...
    my $arr = Blog::Base::String->new($str)->extractKeyVal;
...
sub expand {
    ...
    if ($format =~ /^e?html/) {
        ...
        if ($inlineSegments) {
            ...
            $val =~ s|B\{([^}]+)\}|Blog::Base::Pod->inlineSegment('B',$1)|gse;
            $val =~ s|C\{([^}]+)\}|Blog::Base::Pod->inlineSegment('C',$1)|gse;
            $val =~ s|E\{([^}]+)\}|Blog::Base::Pod->inlineSegment('I',$1)|gse;
            $val =~ s|I\{([^}]+)\}|Blog::Base::Pod->inlineSegment('I',$1)|gse;
            $val =~ s|Q\{([^}]+)\}|"$1"|gs;
        }
    }
    elsif ($format eq 'man') {
