...
Blog::Base::Path - Dateisystem-Operationen
...
sub copy {
    my ($class,$srcPath,$destPath) = @_;

    my $fh1 = Blog::Base::FileHandle->new('<',$srcPath);
    my $fh2 = Blog::Base::FileHandle->new('>',$destPath);
    while (<$fh1>) {
...
sub read {
    ...
    if ($autoDecode) {
        $data = Blog::Base::String->autoDecode($data);
    }
...
sub write {
    ...
    if ($recursive) {
        my $dir = (Blog::Base::Path->split($file))[0];
        if ($dir && !-d $dir) {
...
sub mkdir {
    ...
    if (@_) {
        Blog::Base::Option->extract(-dontExtract=>1,\@_,
            -forceMode=>\$forceMode,
            -mode=>\$mode,
    ...
    if ($recursive) {
        my $parentDir = ($class->split($dir))[0];
        $class->mkdir($parentDir,@_,-mustNotExist=>0);
    }
...
=head4 Example
    ...
    Blog::Base::Path->symlinkRelative('a','x')
    # x => a
    
    Blog::Base::Path->symlinkRelative('a/b','x')
    # x => a/b
    
    Blog::Base::Path->symlinkRelative('a/b','x/y')
    # x/y => ../a/b
    
    Blog::Base::Path->symlinkRelative('a/b','x/y/z')
    # x/y/z => ../../a/b
...
sub symlinkRelative {
    ...
    if ($path !~ m|^/| && $symlink =~ m|/|) {
        if ($symlink !~ m|^/|) {
            ...
            $path = sprintf '%s/%s',Blog::Base::Process->cwd,$path;
        }
    }
    if ($verbose) {
        print "$symlink => $path\n";
