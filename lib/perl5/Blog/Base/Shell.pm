...
Blog::Base::Shell - Ausführung von Shell-Kommandos
...
Instanziiere ein Shell-Objekt, und liefere eine Referenz auf dieses
Objekt zurück.
...
sub exec {
    ...
    unless ($dryRun) {
        ...
        if ($log && $self->{'time'}) {
            ...
            # printf $fd "/* %.2f */\n",$t1-$t0;
            printf "%s%s\n",$self->{'timePrefix'},
                Blog::Base::Converter->epochToDuration($t1-$t0,1,3);
        }
        if ($except) {
