...
Blog::Base::Shell - Klasse zur Ausführung von Shell-Kommandos
...
sub exec {
    ...
    unless ($dryRun) {
        ...
        if ($log && $self->{'time'}) {
            ...
            # printf $fd "/* %.2f */\n",$t1-$t0;
            printf "%s%s\n",$self->{'timePrefix'},
                Blog::Base::Math->epochToDuration($t1-$t0,1,3);
        }
        if ($except) {
    ...
    if ($capture) {
        if ($capture eq 'stderr+stdout') {
            my $stdout = Blog::Base::Path->read($stdoutFile,-delete=>1);
            my $stderr = Blog::Base::Path->read($stderrFile,-delete=>1);
...
Prüfe den Ausführungsstatus von system():
    ...
    system($cmd);
    Blog::Base::Shell->checkError($?,$!,$cmd);
