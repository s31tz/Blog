...
use Blog::Base::Quiq::Ipc;
use Blog::Base::Quiq::String;
use Blog::Base::Quiq::Html::Listing;
...
sub new {
    ...
    $text =~ s/^\|//m;
    $text = Blog::Base::Quiq::Unindent->trim($text);
...
sub dump {
    ...
    if (my $file = $self->{'file'}) {
        $text = Blog::Base::Quiq::Path->read($file);
    }
    elsif (my $execCmd = $self->{'exec'}) {
        if ($execCmd =~ s/%FORMAT%/$format/g) {
            $esc = 0;
        }
        ($text) = eval {Blog::Base::Quiq::Ipc->filter($execCmd,undef)};
        if ($@) {
    ...
    Blog::Base::Quiq::String->removeIndentation(\$text);

    if (my $filterCmd = $self->{'filter'}) {
        if ($filterCmd =~ s/%FORMAT%/$format/g) {
            $esc = 0;
        }
        ($text) = eval{Blog::Base::Quiq::Ipc->filter($filterCmd,$text)};
        if ($@) {
    ...
    if ($h && $self->{'bg'} == 2) {
        return Blog::Base::Quiq::Html::Listing->html($h,
            colNumbers=>$self->{'cn'},
