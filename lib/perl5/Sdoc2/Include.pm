...
sub new {
    ...
    if ($inp =~ /%FORMAT%/ || $execCmd =~ /%FORMAT%/) {
        ...
        if ($execCmd) {
            my ($text) = eval{Blog::Base::Quiq::Ipc->filter($execCmd)};
            if ($@) {
        ...
        my $incDoc = Blog::Base::Quiq::LineProcessor->new($inp,
...
sub dump {
    ...
    if ($file) {
        ...
        $str = Blog::Base::Quiq::Path->read($file);
    }
    elsif ($exec) {
        $exec =~ s/%FORMAT%/$format/;
        ($str) = eval {Blog::Base::Quiq::Ipc->filter($exec)};
        if ($@) {
