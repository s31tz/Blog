...
use Blog::Base::Quiq::LineProcessor;
use Blog::Base::Quiq::Hash;
use Blog::Base::Quiq::Hash::Ordered;
...
sub new {
    ...
    if (!ref $inp) {
        ...
        if (-e $program) {
            ...
            Blog::Base::Quiq::Shell->exec("(cd $dir; ./$program)");
        }
    }

    # Zunächst LineProcessor-Dokument instantiieren

    my $doc = Blog::Base::Quiq::LineProcessor->new($inp,
    ...
    my $self = $class->SUPER::new(
        ...
        parent=>undef,
        childs=>[],
        links=>Blog::Base::Quiq::Hash->new->unlockKeys, # für Link-Definitionen
        anchorsGlob=>Blog::Base::Quiq::Hash::Ordered->new,
        generateAnchors=>1,
        html4=>$html4,
        tableAndFigureNumbers=>$tableAndFigureNumbers,
        centerTablesAndFigures=>$centerTablesAndFigures,
...
sub dump {
    ...
    my (@args,$h);
    if ($format =~ /^e?html$/) {
        push @args,$h = Blog::Base::Quiq::Html::Tag->new(
    ...
    if ($format eq 'debug') {
        ...
        if ($styleSheet) {
            for my $s (split /[,\s]+/,$styleSheet) {
                $style .= "\n" if $style;
                if ($s =~ s/^inline://) {
                    $style .= Blog::Base::Quiq::Path->read($s);
