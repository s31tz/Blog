...
sub nextType {
    ...
    return ($type,Blog::Base::Quiq::Array->new($arr));
}
...
sub parseObjectSpec {
    ...
    my $arr = Blog::Base::Quiq::Converter->stringToKeyVal($str);
...
sub graphicSegment {
    ...
    my $attH = Blog::Base::Quiq::Hash->new(
        width=>undef,
        height=>undef,
        title=>undef,
        anchor=>undef,
...
sub expand {
    ...
    if ($format =~ /^e?html/) {
        ...
        if ($inlineSegments) {
            ...
            $val =~ s|B\{([^}]+)\}|Blog::Base::Quiq::Pod::Generator->fmt('B',$1)|gse;
            $val =~ s|C\{([^}]+)\}|Blog::Base::Quiq::Pod::Generator->fmt('C',$1)|gse;
            $val =~ s|E\{([^}]+)\}|Blog::Base::Quiq::Pod::Generator->fmt('I',$1)|gse;
            $val =~ s|I\{([^}]+)\}|Blog::Base::Quiq::Pod::Generator->fmt('I',$1)|gse;
            $val =~ s|Q\{([^}]+)\}|"$1"|gs;
        }
    }
    elsif ($format eq 'man') {
